...
describe('Controller: NotificationsCtrl', function () {
    ...
    beforeEach(function() {
        ...
        inject(function ($controller, $rootScope, $state, _$httpBackend_, $http, _$interval_) {
            ...
            $httpBackend.when('GET', new RegExp('app/search/results.partial.html'))
                .respond(200, 'some text');
            $httpBackend.when('GET', new RegExp('app/search/search-results.partial.html'))
                .respond(200, 'some text');
    ...
    it('should call runQuery with query param and callSubmit set to true', function() {
        ...
        expect(state.go).toHaveBeenCalledWith('main.search.results.list', {query: queryResults[0], callSubmit: true}, {location: true});
    });
